...
NFA create_Concat(NFA a, NFA b)
{
	...
	NFA_add_transition(ret, a->num_states, 0, a->num_states+1);
	for(int i=0; i<new_size; i++)
	{
		for(int j=0; j<128; j++)
		{
			...
			if(i>=a->num_states && i<b->num_states)
...
NFA create_Union(NFA a, NFA b)
{
	...
	NFA_add_transition(ret, 0, 0, a->num_states+2);
	NFA_add_transition(ret, a->num_states+1, 0, a->num_states+b->num_states+2);
	NFA_add_transition(ret, a->num_states+b->num_states+1, 9, a->num_states+b->num_states+2);
	for (int i = 0; i < new_size;i++) {
		for (int j = 0; j<128; j++) {
